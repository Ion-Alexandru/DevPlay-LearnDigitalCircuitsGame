...
public class XNORGate : ElectricalComponent
{
    public static TilemapClick tilemapClick;
    private bool lastPoweredState = false;
    private Tutorial_1Click tilemapClickInstance;
    public XNORGate(Tilemap tilemap, Vector3Int position, TilemapClick tilemapClickInstance) : base(tilemap, position)
    {
        ...
        tilemap.SetTile(position + Vector3Int.down, tilemapClick.XNORGate);
        tilemap.SetTile(position + 2*Vector3Int.down, tilemapClick.XORGate3);
    }

    public XNORGate(Tilemap tilemap, Vector3Int position, Tutorial_1Click tilemapClickInstance) : base(tilemap, position)
    {
    ...
    public override void UpdateState(Dictionary<Vector3Int, ElectricalComponent> components)
    {
        ...
        if(input1Powered == true && input2Powered == true)
        {
            isPowered = true;
        } 
        else if(input1Powered == false && input2Powered == false)
        {
            isPowered = true;
        }
        else
